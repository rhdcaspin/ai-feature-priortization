...
Manual overrides (skipped on future runs):
  - Jira label ``rice-rank-manual`` (configurable via JIRA_RANK_MANUAL_LABEL), or
...
def sort_keys_by_rice_desc(
    ...
    """Highest RICE first; missing RICE last; RICE 0.0 just above missing."""
...
def sort_keys_by_rice_asc(
    ...
    no RICE / 0 at the bottom, highest RICE at the top.
    """

    def sort_key(k: str) -> Tuple[int, float, str]:
        score = rice_by_key.get(k)
        if score is None:
            return (0, 0.0, k)  # bottom
        if abs(score) <= RICE_EPSILON:
            return (1, 0.0, k)
        return (2, score, k)  # ascending positive scores
...
def apply_rice_rank_order(
    ...
    *,
    ...
    """
    Order Rank to match RICE (high → low).

    **asc** (default): Jira Product Discovery global plan / ``ORDER BY Rank ASC``.
    Scored keys (any RICE value, including 0) are chained ``rankAfterIssue`` from the plan
    tail in ``sort_keys_by_rice_desc`` order (highest RICE → row #1). Each no-RICE key is
    then moved ``rankAfter`` a refreshed plan tail so they stay at the bottom.
    ...
    **desc**: ``ORDER BY Rank DESC`` boards; batched ascending before refreshed Rank-DESC #1.
    ...
    rice_keys_asc = sort_keys_by_rice_asc(positive_rice_keys, rice_by_key)
    no_rice_keys = sort_keys_by_rice_asc(
        [k for k in keys if rice_by_key.get(k) is None], rice_by_key
    )
    ...
    if view == "asc":
        scored_chain = sort_keys_by_rice_desc(
            [k for k in keys if rice_by_key.get(k) is not None], rice_by_key
        )
...
def count_rice_rank_misalignment(
    ...
    """Count adjacent pairs where higher lexorank issue has lower RICE than neighbor below."""
    if len(keys) < 2:
        return 0
    ...
    if ordered_keys:
        ...
        if r.status_code == 200:
            ...
            for iss in r.json().get("issues") or []:
                k = iss.get("key")
                if k:
                    fresh_rice[k] = _parse_rice_score((iss.get("fields") or {}).get(rice_field))
...
def main() -> int:
    ...
    parser.add_argument(
        "--force-rank",
        action="store_true",
        help="Ignore manual-override locks for this run (still respects rice-rank-manual label)",
    )
    parser.add_argument(
        "--rank-view",
        choices=("asc", "desc"),
    ...
    for key in rice_by_key:
        entry = issue_state_entry(bucket, key)
        if (
            not getattr(args, "force_rank", False)
            and (
                entry.get("manual_override")
                or has_manual_label(labels_by_key.get(key, []), args.manual_label)
            )
        ):
    ...
    ascending = sort_keys_by_rice_asc(auto_keys, rice_by_key)
    ...
    print(f"   Ranked by RICE (incl. no-score at bottom): {len(ordered)}")
    ...
    for i, key in enumerate(ordered[:25], 1):
        print(f"      {i:3}. {key}  RICE={rice_by_key[key]}")
    if len(ordered) > 25:
    ...
    if misaligned:
        print(f"   ⚠️  {misaligned} issue(s) still out of RICE order among ranked set — re-run or check manual locks")
    else:
        print("   ✅ LexoRank order matches RICE order for ranked issues")
